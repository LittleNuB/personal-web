...
The personal-site route keeps only assets needed by the interactive demo. The presentation-only background music is excluded.
...
The static route contains no API key, server proxy, real training record, source concept board, test output, or local log.
